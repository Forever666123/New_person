...
**不用时刻表，也不用"今天不理人"这种开关。** 长延迟应该是几件事撞在一起的自然结果。四层：

**第一层，学期日历**（`calendar.py`）。留学生的一年不是均匀的：开学、期末周、感恩节、
寒假、春假、暑假，每一段的作息、心情、有没有课都不同。persona.yaml 里填的是
Northeastern 2026–2027 的真实日期，超出范围的年份按典型模式推算，跑过一学年也不会没日历。

放假会**出门**。假期越长走得越远，出行是确定性生成的，所以"上周去了哪"不会问一次变一次。
出门期间她**人在别的时区**，作息整个跟着那边走，在外面玩手机也看得少。
这一条顺带解决了一个真实感难题：她回国的时候和悉尼只差两小时，回消息忽然就快了。

**第二层，阶段（phase）**，跨三到十几天。赶 due 一忙就是一整周，不是每天重新掷骰子。
只在上课期间可能抽到"赶 due"，放假不会。

**第三层，当日变体（variant）**：普通 / 熬夜 / 早起 / 累 / 闲 / 心情差。
变体影响的是**当天**：抽到熬夜就是今晚睡得晚、今天起得晚。最极端的一档权重只有 4，
而且那天她仍然会看手机。**没有任何一天她是完全关机的。**

**第四层，看手机（glance）**，见 2.2。

睡觉时刻每天从分布采样，且**起床跟着昨晚走**（`sleep_follow_weight`）：熬夜到四点不会
七点就起，但也不会一路睡到下午，因为有课有闹钟。假期 `sleep_bonus_hours` 让她睡得更久，
期末周更少。每节课按自己的 `probability` 决定今天去没去，放假和期末周自动没课。

抽签的随机源是 `(persona.seed, 日期)`，所以同一天反复查询结果一致，进程重启也一致，
但不同的日子互不相关。

于是"她一整天没怎么回"通常是：这周在赶 due，昨晚熬到四点，
他刚好在她睡着的时候发的消息，醒来又赶上有课。四件事叠出来的，不是开关。

**活跃度曲线**取代了 busy 开关。`activity_at(dt)` 返回此刻"会看手机"的程度，0 到 1：
睡着是 0，上课是 `class_activity`，其余按 `activity.points` 阶梯取值，
最后乘当日变体、阶段、学期、出行的倍率。它只决定多久瞄一眼手机，不决定回不回。
...
- hot（手机就在手上）：`notice_at = now`，`reply_at = now + LN(hot_reply_median_seconds, σ)`，
  默认中位数 75 秒。**没有秒回这回事**：手机拿起来放下、打字、被别的事岔开，
  三五分钟才回一句是常态。
...
- **能机械修的**直接修，不惊动模型：句尾句号去掉，句中句号变空格（等于换口气继续说），
  emoji 和感叹号削到预算内，气泡超量就合并而不是丢内容。
- **不能机械修的**返回违规清单，让模型带着清单重写一次：说了 `never_say` 里的话、
  整句英文、长句超预算。重写还不过就退回机械修剪，不会卡住。

**用预算，不用开关。** 年轻人不可能一个 emoji 都不用，偶尔一个是正常的，满屏才不正常。
所以 emoji、感叹号、长句都按预算判定：频率交给提示词描述，这里只管上限。
把某个预算设成 0 就等于完全禁止，给别的人设留口子。

其余几个刻意的宽松处：问号保留，她问问题是表达在意的方式；
短的英文句子放过，只拦整段英文，因为她在美国待了好几年，夹英语是常态；
长句超预算才算违规，"极少数超过二十个字的句子"本身是人设的一部分，出现时说明是重话。
...
**记忆是会淡的，这是故意的。** 一个什么都记得的人，聊天就没意思了。
`facts` 表带 `strength`，按 `fact_half_life_days` 随时间衰减，被提起时重新增强。
构造上下文时只带强度高于 `fact_recall_threshold` 的事实，低于的等于想不起来了。
她记不清就直接问，不装作记得。她也不打听：他不说的，她不会去问。
